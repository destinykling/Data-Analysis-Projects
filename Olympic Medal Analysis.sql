...
-- what sport earned the most gold medals
    ...
    -- what sport is each country best at
 SELECT country, MAX(Medal_Count) AS Most_Gold_Medals
FROM (
    SELECT country, sport, COUNT(Medal) AS Medal_Count
    FROM olympics
    WHERE medal = 'Gold'
    GROUP BY country, sport
) AS GoldMedalsPerSport
GROUP BY country;
